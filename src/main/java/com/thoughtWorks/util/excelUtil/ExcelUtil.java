...
public class ExcelUtil {
    ...
    public static <T> void exportExcel(Map<String, String> headers, Collection<T> dataset, File file) {
        exportExcel(headers, dataset, file, null);
    }
    ...
    public static void exportExcel(String[][] datalist, File file) {
        OutputStream out = null;
        try {
            out = new FileOutputStream(file);
        } catch (FileNotFoundException e) {
...
//                        cell.setCellValue(String.valueOf(value));
    ...
    public static <T> Collection<T> importExcel(Class<T> clazz, File file,
        ...
        try {
            if (getSuffix(file).equals("xls")) {
                xlsBook = new HSSFWorkbook(new FileInputStream(file));
                xlsSheet = xlsBook.getSheetAt(0);
                rowIterator = xlsSheet.iterator();
            } else if (getSuffix(file).equals("xlsx")) {
                xlsxWorkbook = new XSSFWorkbook(new FileInputStream(file));
