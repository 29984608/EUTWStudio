...
public class DirectionReportUtil extends ExcelReportUtil {
    @Override
        ...
        try {
            ...
            for (int i = 0; i < dataset.size(); ++i) {
                ...
                if (directions.size() >= 2)
                    sheet.addMergedRegion(new CellRangeAddress(rowIndex, rowIndex + directions.size() - 1, 1, 1));
                ...
                for (int j = 0; j < directions.size(); ++j) {
                    ...
                    cell.setCellValue(new HSSFRichTextString(directions.get(j).get("directionName").toString()));
